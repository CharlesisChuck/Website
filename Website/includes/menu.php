
<li class="menu-link"><a class="slideBack" href="?page=home">Home</a></li>

<li class="menu-link">
  <a href="#">Learn</a>
  <ul class="submenu">
   <li><a href="?page=learn-creator&page_category=other">Other</a></li>
   <li><a href="?page=learn-creator&page_category=book">Books</a></li>
   <li><a href="?page=learn-creator&page_category=buissness">Buissness</a></li>
   <li><a href="?page=learn-creator&page_category=computer-science">Computer Science</a></li>
   <li><a href="?page=learn-creator&page_category=physics">Physics</a></li>
   <li><a href="?page=learn-creator&page_category=mechanics">Mechanics</a></li>
   <li><a href="?page=learn-creator&page_category=electronics">Electronics</a></li>
   <li><a href="?page=classes-completed">Completed Classes</a></li>
   <li><a href="?page=classes-current">Current Classes</a></li>
   <li><a href="?page=classes-overview">Overview</a></li>
  </ul>
</li>

<li class="menu-link">
  <a href="#">Feynman</a> 
  <ul class="submenu">
    <li><a href="?page=learn-creator&page_category=feynman">Book Lessons</a></li>
    <li><a href="?page=feynman-learn">What I Learned</a></li>
    <li><a href="?page=feynman-teach">His Teachings</a></li>
    <li><a href="?page=feynman-who">Who is Feynman?</a></li>
  </ul>  
</li>

<li class="menu-link">
  <a href="#">Portfolio</a> 
  <ul class="submenu">
    <li><a href="?page=#"></a></li>
    <li><a href="?page=portfolio-questions">Questions for Me</a></li>
    <li><a href="?page=portfolio-references">References</a></li>
    <li><a href="?page=portfolio-about">About</a></li>
    
    <li><a href="?page=portfolio-resume">Resume</a></li>
  </ul>
</li>

<li class="menu-link">
  <a href="?page=schedule">Schedule</a> 
</li>

<li class="menu-link">
  <a href="#">Extra</a> 
  <ul class="submenu">
    <li><a href="?page=extra-websites">Other Websites</a></li>
    <li><a href="?page=extra-music">Favorite Music</a></li>
    <li><a href="?page=extra-blog">Blog</a></li>
  </ul>
</li>

<li class="menu-link">
  <a href="#">Admin</a> 
  <ul class="submenu">
    <li><a href="?page=admin-delete">Delete Content</a></li>
    <li><a href="?page=admin-edit">Add Content</a></li>
    <li><a href="?page=admin-login">Create UserID</a></li>
    <li><a href="?page=TESTPAGE">TESTPAGE</a></li>
  </ul>
</li>

<li class="menu-link"><a href="?page=contacts">Contact</a> </li>
